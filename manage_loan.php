...
<div class="container-fluid">
	...
	<form  id="loan-application">
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Borrower/ተበዳሪ</label>
				<?php
				$borrower = $conn->query("SELECT *,concat(firstname,', ',middlename,' ',lastname) as name FROM borrowers order by concat(firstname,', ',middlename,' ',lastname) asc ");
				?>
				<select name="borrower_id" id="borrower_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $borrower->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($borrower_id) && $borrower_id == $row['id'] ? "selected" : '' ?>><?php echo $row['name'] . ' | Tin no:'.$row['tax_id'] ?></option>
						<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-6">
				<label class="control-label">Loan Type/የብድር አይነቶች</label>
				<?php
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Loan Plan/የብድር እቅድ</label>
				<?php
		...
		<div class="form-group col-md-6">
			<label class="control-label">Loan Amount/የብድር መጠን</label>
			<input type="number" name="amount"  class="form-control text-right" step="any" id="amount" value="<?php echo isset($amount) ? $amount : '' ?>">
		</div>
		</div>
		<div class="row">
			<div class="form-group col-md-6">
			<label class="control-label">Purpose/ዐላማ</label>
			<textarea name="purpose" id="purpose" cols="30" rows="2"  class="form-control"><?php echo isset($purpose) ? $purpose : '' ?></textarea>
		</div>
...
<script>
	...
	function validateForm(){
	 	...
	 	if($('#amount').val() == ''){
	 		valid = false;
	 		alert_toast("amount is required.","warning");
	 	}
		 if($('#purpose').val() == ''){
        valid = false;
        alert_toast("purpose is required.","warning");
    }
	return valid;
}
</script>
